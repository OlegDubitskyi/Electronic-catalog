...
class CDatabase {
	...
	function query($sql) {	//is
		if(!$this->link) return 0;
		return mysql_query($sql, $this->link);
	}

	function affected_rows() { //no
		return mysql_affected_rows($this->link);
	}

	function num_rows($q) {  //is
		return mysql_num_rows($q);
	}

	function fetch_array($q, $result_type=MYSQL_ASSOC) { //is
		return mysql_fetch_array($q, $result_type);
	}

	function fetch_object($q) {   // no
		return mysql_fetch_object($q);
	}

	function data_seek($q, $n) { //no
		return mysql_data_seek($q, $n);
	}

	function free_result($q) { //no
		return mysql_free_result($q);
	}

	function insert_id() { // is
	...
	function sql2array($sql, $keyField='') { //no
		if((empty($sql)) || (!($query = $this->query($sql)))) return false;
		if($this->num_rows($query) < 1) return false;
		return $this->result2array($query, $keyField);
	}

	function result2array($q, $keyField='') { //no
		$Result = array();
	...
	function list_tables() { //no
		return mysql_list_tables($this->db, $this->link);
	}

	function list_fields($table_name) { // no

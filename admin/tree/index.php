...
#########################################################
//require_once($path_lib."/template.inc");
//$t = new Template("$path_template/admin");
//$t->set_file(array("page" => "index.htm"));
//$t->set_root("$path_template/admin/tree");
//$t->set_file("main","tree.htm");
...
include_once($path_lib."/lan/lan.php");
$nlan="eng.txt";
$l = new lan ("lan/".$nlan);
$words=$l->getW();
...
if ($type_but=="ren" and isset($id_cat)){
	//$move=$tree->delete($id_cat);
	$adduser=$db->query("update $table set cat_name='$rename' where cat_id='$id_cat'");
	$tree->gen_menu();
	//include('Tree_Menu.php');
	//include('../../inc/Tree_Menu.php');
	header("Location: index.php?open_it=$open_it");
